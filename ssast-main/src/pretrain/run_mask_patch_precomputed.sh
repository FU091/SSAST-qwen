...
#SBATCH -n 1
#SBATCH --mem=48000
#SBATCH --job-name="ssast_pretrain_precomputed"
...
# For precomputed dataset: point to the directories containing .pt files
dataset=precomputed
# Point to your training and validation .pt file directories
tr_data="../combined_train_data.json"  # JSON file mapping original audio paths to labels (used for mapping to .pt files)
te_data="../test.json"  # JSON file for validation (optional)
data_dir="/D/spectrogram_pt_name"        # Directory containing .pt files for training
data_val_dir="/D/val_spectrogram_pt_name"   # Directory containing .pt files for validation
...
--data-train ${tr_data} --data-val ${te_data} --data_dir ${data_dir} \
--exp-dir $exp_dir \
