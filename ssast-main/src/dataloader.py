...
class AudioDataset(Dataset):
    ...
    def _wav2fbank(self, filename, filename2=None):
        def safe_load(path, sr_expected=None, max_frames=1600000):
            """安全讀取 wav，避免一次性載入過大檔案"""
            ...
            if num_frames > max_frames:
                ...
                while offset < num_frames:
                    frames_to_read = min(max_frames, num_frames - offset)
            ...
            if sr_expected is not None and sr != sr_expected:
                waveform = torchaudio.functional.resample(waveform, sr, sr_expected)
                sr = sr_expected

            return waveform - waveform.mean(), sr

        # 單檔情況
        if filename2 is None:
            waveform, sr = safe_load(filename)
        else:
            waveform1, sr = safe_load(filename)
            waveform2, _ = safe_load(filename2, sr_expected=sr)
