...
if args.dataset == 'precomputed':
    ...
    # For precomputed dataset: args.data_train is the directory, and we also use JSON mapping
    # The directory contains .pt files and we need the original JSON to map index to filenames
    # We assume the JSON files are located in the project root (where run.py is executed from)
    import os
    ...
    if os.path.exists(train_json_path):
        ...
        train_dataset = PrecomputedDataset(args.data_train, dataset_json_file=train_json_path)
    else:
        print(f"Training JSON file not found: {train_json_path}, using directory-only approach")
        # Fallback: we need to handle the case where no JSON is available
        train_dataset = PrecomputedDataset(args.data_train)

    if os.path.exists(val_json_path):
        print(f"Using validation JSON file for mapping: {val_json_path}")
        val_dataset = PrecomputedDataset(args.data_val, dataset_json_file=val_json_path)
    else:
        print(f"Validation JSON file not found: {val_json_path}, using directory-only approach")
        val_dataset = PrecomputedDataset(args.data_val)
